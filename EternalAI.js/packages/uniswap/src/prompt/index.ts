...
import { PromptPayload } from './type';
...
export const uni_swap_ai = async (
  command: string,
  args: any
): Promise<TTransactionResponse | null> => {
...
export const prompt = async (payload: PromptPayload) => {
  console.log('🚀 ~ prompt ~ payload:', payload);
  const { private_key, model, api_key, host } = payload;
  // Your code here
  try {
