...
export class AgentInference {
  ...
  create_inference_agent = async (
    ...
    if (await this.web3_provider.getNetwork()) {
      ...
      const call_data = agent_contract.interface.encodeFunctionData('prompt', [
        prompt_data,
      ]);
      ...
      const tx_receipt = await waitForTransactionReceipt(
        this.web3_provider,
        tx.hash,
        3
      );
...
export class InferenceProcessing {
  ...
  get_assignments_by_inference = async (
    worker_hub_address: string,
    inference_id: string,
    rpc: string
  ) => {
    this.create_web3_provider(rpc);
    if (await this.web3_provider.eth.net.isListening()) {
      ...
      const worker_hub_contract = new this.web3_provider.eth.Contract(
        WORKER_HUB_ABI,
        this.workerhub_address
      );
      const assignments_info = await worker_hub_contract.methods
        .getAssignmentsByInference(inference_id)
        .call();
      for (const assignment of assignments_info) {
        const assignment_info = await worker_hub_contract.methods
  ...
  get_inference_by_inference_id = async (
    worker_hub_address: string,
    inference_id: number,
    rpc: string
  ) => {
  ...
  process_output_to_infer_response = async (output: any) => {
    ...
    if (!infer_reponse) {
      ...
      if (
        infer_reponse.storage == 'lighthouse-filecoint' ||
        infer_reponse.result_uri.includes('ipfs://')
      ) {
        const light_house = infer_reponse.result_uri.replace(
          IPFS,
          LIGHTHOUSE_IPFS
        );
  ...
  get_infer_id = async (
    ...
    if (await this.web3_provider.getNetwork()) {
      ...
      this.get_workerhub_address(worker_hub_address);
      const tx_receipt = await this.web3_provider.getTransactionReceipt(
        tx_hash_hex
      );
